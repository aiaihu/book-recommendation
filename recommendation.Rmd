...
## Business Objective
The objective for this project is to builde a book recommender system which could suggest books to users based on what other book lover who have read and rated in the past.
...
######to_read.csv
Contains 912705 observations(rows) and 2 variables(columns). This dataset contains user_id and book_id.
...
Let's predict the 20th user entry in the real_ratings dataset.We want to look at the top 5 books that the user will give rating for . For the convenience, we call this user "Galileo"
```{r}
...
We split the first 5000 users in real_ratings dataset into 90/10 (known/unknown)

```{r}
# create 90/10 split (known/unknown) for the first 5000 users in real_ratings
scheme <- evaluationScheme(real_ratings[1:5000,], method = "split", train = .9, k = 1, given = -1, goodRating = 4)
scheme
```
...
From above, we guess the accuracy rate might not be high. Does this due toa small dataset that we are using? I am not quite sure as our RMSE was low.
...
```{r}
#Create initial empty predictions matrix.(df is at line 536)

m <- matrix(sample(c(NA,0:5),ncol(df), replace=TRUE, prob=c(.6,rep(.3/6,6))),
	nrow=1, ncol(df), dimnames = list(
	    user_id =paste(1:1),
	    book_id =paste(1:10000)
    ))
dim(m)
```
...
######Recommend next book for this new user

```{r}
rec_book <-predict(ub_rcmnd,ratings.new, n=1)
rec_num = as(rec_book, "list")
rec_num
```
```{r}
cat('The next recommend book is ', as.numeric(rec_num))
```
## Part 5: Shinny App
In order to have a real book recommender, we will build an app. We will use the UBCF Collaborative Filtering system. Our app will recommend the top 5 books for an existing user based on predicted ratings that the user will be given on these books. Due to the dataset which we used in this project is very larg, we will only subsample 20% of the rating dataset to increase computation speed. After your put an user_id which is existing in the ratings dataset, the recommender system will tell you the top five books that the user will probably like it.
...
* We would suggest to get more information about user profiles such as age, education background, culture background etc. to do more deep analying as there are many factors in people's reading taste.
