...
In above, we see user_based CF algrothms preform better prediction than random, and RMSE increase slightly when increasing number of nearest neighbours nn.
...
```{r}
#Create initial empty predictions matrix.(1 user and 10000 books)

m <- matrix(sample(c(NA,0:5),10000, replace=TRUE, prob=c(.6,rep(.3/6,6))),
	nrow=1, ncol=10000, dimnames = list(
	    user_id =paste(1:1),
...
*Based on RMSE number, the is model gives good accuracy. We can use this model to recommend books, movies,shows or some other items for online shopping. However, it requests a large dataset to increase the accuracy.

*We compared model with different alogrithms and found "UBCF_Pearson" is the best one for this dataset. The computation time is fast and RMSE is the smallest one.

* As one user can rate many books or one book can have many user to rate it, therefore, when we do the dataset split, it could bring bias into the train and test dataset.
